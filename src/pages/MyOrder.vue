<template>
  <div class="bg-page font14 user-list-page">
    ...
    <div class="s-container scroll-container myorder-page" style="top:44px;" ref="scrollContainer" @scroll="handleScroll('scrollContainer',0)">
...
export default {
  ...
  methods: {
    ...
    handleEdit (row) {
      let id = row.id
      if (row.status === 4) {
        this.$router.push({path: '/fenJing', query: {id: id, type: this.clickType}})
      } else {
...
<style lang="less">
.user-list-page{
  .myorder-page .el-table__row,.vux-tab-item{
    cursor: pointer;
  }
}
</style>
